...
async function initFlatpickrFields(){try{const flatpickr=await ensureFlatpickr();const locale=window.flatpickr?.l10ns?.ru||'default';document.querySelectorAll('input[type="date"], input[data-flatpickr-date]').forEach(input=>{if(input._flatpickr)return;input.type='text';input.dataset.flatpickrDate='';input.placeholder='Выберите дату';const dialog=input.closest('dialog');flatpickr(input,{locale,dateFormat:'d.m.Y',minDate:'today',disableMobile:true,allowInput:false,clickOpens:true,appendTo:dialog||document.body,positionElement:input});});document.querySelectorAll('input[type="time"], input[data-flatpickr-time]').forEach(input=>{if(input._flatpickr)return;input.type='text';input.dataset.flatpickrTime='';input.placeholder='Выберите время';const dialog=input.closest('dialog');flatpickr(input,{locale,enableTime:true,noCalendar:true,dateFormat:'H:i',time_24hr:true,minuteIncrement:15,disableMobile:true,allowInput:false,clickOpens:true,appendTo:dialog||document.body,positionElement:input,defaultHour:10,defaultMinute:0,onChange(selectedDates,dateStr,instance){if(!dateStr||isOpenTime(dateStr)){setError(input,'');return}const [hours,minutes]=dateStr.split(':').map(Number);const total=hours*60+minutes;const replacement=total>OPEN_END_MINUTES&&total<OPEN_START_MINUTES?(total<5*60?'01:00':'10:00'):'10:00';instance.setDate(replacement,false,'H:i');input.dispatchEvent(new Event('input',{bubbles:true}));setError(input,'')}});});}catch(error){console.error('flatpickr failed',error)}}
